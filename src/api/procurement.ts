...
export async function listSuppliers(page=1,perPage=25){return (await http.get<Paged<Supplier>>(`/suppliers?page=${page}&per_page=${perPage}`)).data}
...
export async function listPurchaseOrders(page=1,perPage=25){return (await http.get<Paged<PurchaseOrder>>(`/purchase-orders?page=${page}&per_page=${perPage}`)).data}
