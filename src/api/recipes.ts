...
export async function listRecipes(page=1){return (await http.get<Paged<Recipe>>(`/recipes?page=${page}&per_page=25`)).data}
...
export async function listMeals(customerId:number,page=1){return (await http.get<Paged<Meal>>(`/customers/${customerId}/meals?page=${page}&per_page=25`)).data}
